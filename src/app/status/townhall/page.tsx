...
export default function SystemStatus() {
  const componentName = "Status_TownHall"
  ...
  const boolVal = (val: string | null) => val === "true"
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-gray-900 rounded-xl shadow-lg text-gray-100 font-sans">
      ...
      {/* Boolean Status */}
      <div className="bg-gradient-to-br from-green-900 to-green-700 rounded-xl p-6 shadow-lg max-w-md mx-auto">
        <h2 className="text-xl font-semibold mb-6 text-center border-b border-green-500 pb-3">Power Status</h2>
        <BooleanStatus label="System AUX Power" isOn={boolVal(systemAuxPower.state)} />
        <BooleanStatus label="PoE++ Status A" isOn={boolVal(poePlusPlusA.state)} />
        <BooleanStatus label="PoE Status A" isOn={boolVal(poeStatusA.state)} />
      </div>
    </div>
  )
}
...
function BooleanStatus({ label, isOn }: { label: string; isOn: boolean }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-green-500 last:border-b-0">
      <span className="font-medium">{label}</span>
      <span
        className={`text-sm font-bold px-3 py-1 rounded-full select-none
          ${isOn ? "bg-green-400 text-green-900" : "bg-red-500 text-red-100"}`}
      >
        {isOn ? "ON" : "OFF"}
      </span>
    </div>
